...
image = (modal.Image.from_registry("nvidia/cuda:12.4.0-devel-ubuntu22.04", add_python="3.12")
    ...
    .run_commands(["mkdir -p /usr/share/fonts/truetype/custom", "wget -O /usr/share/fonts/truetype/custom/Anton-Regular.ttf https://github.com/google/fonts/raw/main/ofl/anton/Anton-Regular.ttf", "fc-cache -f -v"])
    .add_local_dir("asd", "/asd", copy=True))

app = modal.App(name="ai-podcast-clipper", image=image)
...
def create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, output_path, frame_rate=25):
    ...
    for fidx, fname in tqdm(enumerate(flist), total=len(flist), desc="Creating vertical video"):
        ...
        if vout is None:
            vout = ffmpegcv.VideoWriterNV(file=temp_video_path, codec=None, fps=frame_rate, resize=(target_width, target_height))
        ...
        if mode == "resize":
            ...
            cropped_background = blurred_background[center_y:center_y + resized_height, :]
            ...
            center_x = int(max_score_face['x'] * scale if max_score_face else frame_width // 2  ) # type: ignore
...
def create_subtitles_with_ffmpeg(transcript_segments: list, clip_start: float, clip_end: float, clip_video_path: str, output_path: str, max_words: int = 5):
    ...
    new_style.secondarycolour = pysubs2.Color(0, 0, 0, 128) # type: ignore
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    shutil.copy(clip_segment_path, str(pathlib.Path(base_dir) / f"{clip_name}.mp4"))
    ...
    print(f"Columbia took {columbia_end_time - columbia_start_time:.2f} seconds")

    tracks_path = clip_dir / "pywork" / "tracks.pckl"
    scores_path = clip_dir / "pywork" / "scores.pckl"

    if not tracks_path.exists() or not scores_path.exists():
        raise FileNotFoundError("Columbia failed to generate tracks or scores")
    ...
    vertical_video_start_time = time.time()
    create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, vertical_mp4_path)
    vertical_video_end_time = time.time()
    print(f"Vertical video {clip_index} creation took {vertical_video_end_time - vertical_video_start_time:.2f} seconds")
    ...
    s3_client.upload_file(str(subtitle_output_path), "val-ai-podcast-clipper", output_s3_key)
...
class AiPodcastClipper:
    ...
    def load_model(self):
        ...
        self.gemini_client = genai.GenerativeModel("gemini-2.5-flash-preview-04-17")  # type: ignore



    def transcribe_video(self, base_dir: str, video_path: str) -> str:
        audio_path = str(pathlib.Path(base_dir) / "audio.wav")
        ...
        print(f"WhisperX transcription and alignmenttook {duration:.2f} seconds")
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("val-ai-podcast-clipper", s3_key, str(video_path))

        # Transcribe video
        transcript_segments_json = self.transcribe_video(base_dir=str(base_dir), video_path=str(video_path))
        ...
        if not clip_moments or not isinstance(clip_moments, list):
            print("Error: Clip moments is not a list")
            clip_moments = []
        ...
        if base_dir.exists():
            print(f"Cleaning up base directory after {base_dir}")
...
def main():
    ...
    if url is None:
        raise ValueError("Web URL is not available")
